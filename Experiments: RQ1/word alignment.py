...
meteor_wa=[]
for i in range(len(auth_1)):
    meteor_wa.append(round(meteor_score([refs[i]], preds[i]),2))
print(meteor_wa)
...
for a in range(len(meteor_wa)):
    norm_m_wa.append(round(((meteor_wa[a])/(max_m)),2))
print(norm_m_wa)
...
coef, p = spearmanr(norm_m_wa, avg_norm_score)
...
meteor_woa=[]
for i in range(len(auth_1)):
    meteor_woa.append(round(meteor_score_woa([refs[i]], preds[i]),2))
print(meteor_woa)
...
max_m=max(meteor_woa)
min_m=min(meteor_woa)

for a in range(len(meteor_woa)):
    norm_m_woa.append(round(((meteor_woa[a])/(max_m)),2))
print(norm_m_woa)
...
meteorn_wa=[]
for i in range(len(auth_1)):
    meteorn_wa.append(round(meteorn_score_wa([refs[i]], preds[i]),2))
print(meteorn_wa)
...
max_m=max(meteorn_wa)
min_m=min(meteorn_wa)

for a in range(len(meteorn_wa)):
    norm_mn_wa.append(round(((meteorn_wa[a])/(max_m)),2))
print(norm_mn_wa)
...
from scipy.stats import spearmanr
# calculate spearman's correlation
coef, p = spearmanr(norm_mn_wa, avg_norm_score)
print('Spearmans correlation coefficient: %.3f' % coef)
# interpret the significance
alpha = 0.05
if p > alpha:
...
meteorn_woa=[]
for i in range(len(auth_1)):
    meteorn_woa.append(round(meteorn_score_woa([refs[i]], preds[i]),2))
print(meteorn_woa)
...
max_m=max(meteorn_woa)
min_m=min(meteorn_woa)

for a in range(len(meteorn_woa)):
    norm_mn_woa.append(round(((meteorn_woa[a])/(max_m)),2))
print(norm_mn_woa)
...
from scipy.stats import spearmanr
# calculate spearman's correlation
coef, p = spearmanr(norm_mn_woa, avg_norm_score)
print('Spearmans correlation coefficient: %.3f' % coef)
# interpret the significance
alpha = 0.05
if p > alpha:
